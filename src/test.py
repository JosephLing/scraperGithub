                ...
                if fileasstring:
                    ...
                    for yaml_line in yaml_file_lines:
                        ...
                        if isCommentInString(yaml_line):
                            comments += 1
                    comment_stats[key].append((comments/len(yaml_file_lines))*100)

    total_comments = []
    for k in PATHS.keys():
        for v in comment_stats[k]:
            total_comments.append(v)

    data = pd.DataFrame(total_comments)
    px.box(data, y=0, points="all").show()

    data = pd.DataFrame([v for v in total_comments if v != 0])
    px.box(data, y=0, points="all").show()
    # fig = go.Figure()

            # print(.describe())
        # px.scatter(x=comment_stats[k], title=k).show()
        # fig.add_trace(go.Scatter(x=comment_stats[k]))
    # fig.show()
...
def get_spreadofdata():
    ...
    for line in csvfiledata:
        ...
        watchers.append(int(line.get("subscribers_count")))

    xAxis = list(range(len(csvfiledata)))
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=xAxis, y=watchers, name="watch"))
    fig.add_trace(go.Scatter(x=xAxis, y=stars, name="stars"))
    fig.show()

    fig = px.scatter(x=stars, y=watchers)
    fig.show()
