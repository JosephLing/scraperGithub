...
\todo[]{so the cloud bees data isn't in our corpus that they gave us... so hwo do we do comparison based on the data. But the paper says that it's 35,544 which is the size of data we have in our excel doc they gave us.... so :shrug@: but should only effect RQ2 which we can just use the details from the paper to reproduce the table/bar graph}
...
% \todo[]{table references aren't working atm so going to hard code woo! (have googled for solutions but no avail)}
...
\todo[]{remove topn from the 2020 dataset and normalise both graphs to a percentage so that comaprisons can easily be done between them. However how much of a comparison can you make with the datasets being so different}
