...
% this part is turning into an analysis of exisitng papers....
...
\todo[size=\scriptsize]{check which tense to use}
Although we won't be looking at puppet (\todo[size=\scriptsize]{what is puppet???}) research has also be done on the "code smell" which can be an indication on the quality of the code by analyzing its structure \cite{Sharma2016}.

\todo[size=\scriptsize]{check which version of english to use}
Initially this paper started off looking at how to visualize Continuous Integration (CI) and Continuous delivery (CD) systems. However due to their being a lack of research in the area (\todo[size=\scriptsize]{cite the paper that lists all of the papers around the area}) on specifically on how CI is used and practical implementation. A lot of research has been done supporting the use of CI/CD and it's implementation and benefits in cases for particular languages.

% Research queqsiotnsasdf
% - ??? RQ1
